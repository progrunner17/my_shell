...
static process* initialize_process() {
    ...
    p->next = NULL;
    p->state = WAITING;
    p->pipe[0] = p->pipe[1] = -1; //add
    memset(&p->pid, 0, sizeof(pid_t));//add
...
static job* initialize_job() {
    ...
    j->mode = FOREGROUND;
    memset(&j->pgid, 0, sizeof(pid_t));//add
    j->command = NULL;//add
    j->job_num = 0;//add
...
// ジョブリスト全てではなく、個別のジョブの開放に変更

void free_job(job *j) {


    if(!j) return; //NULL -> return
...
job* parse_line(char *buf) {
    ...
    int index=0, arg_index=0,str_i=0;
    ...
    while(*buf != '\n') {
        ...
        /* 状態OUT_REDIRCT_*は出力リダイレクション対象ファイル名を読む状態 */
            ...
            if(!curr_job) {
                ...
                curr_job->command = malloc(strlen(buf)*sizeof(char));//command に入力されたコマンドをコピー
                // strcpy(curr_job->command,buf);
                for(str_i =0;buf[str_i] != '\n';str_i++){
                    if(buf[str_i]=='&') break;
                    (curr_job->command)[str_i] = buf[str_i];
                }
                (curr_job->command)[str_i] ='\0';
